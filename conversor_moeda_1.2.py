...
taxas_conversao = {
    (1, 6): 5.25,    # Dólar para Real
    (2, 6): 5.90,    # Euro para Real
    (3, 6): 6.80,    # Libra para Real
    (4, 6): 0.80,    # Yuan para Real
    (5, 6): 0.0045  # Won para Real
}
...
def conversor_moeda(valor, id_origem, id_destino): 
    ...
    # 1. Busca o nome da moeda e simbolos
    nome_origem = nomes_moedas[id_origem]
    nome_destino = nomes_destino[id_destino]
    simbolo_origem = simbolos_moedas[id_origem]

    # 2. Faz o cálculo
    if(id_origem == 6):  # Se a origem for REAL (6), nós DIVIDIMOS para comprar moeda estrangeira
        # 1. Busca a taxa usando a TUPLA (origem, destino)
        # Exemplo: Se id_origem for 2 e id_destino for 1, busca (2, 1)
        taxa = taxas_conversao[id_destino, id_origem] # Inverte a taxa para conversão de Real para moeda estrangeira 
        valor_final = valor / taxa
    else:
        taxa = taxas_conversao[id_origem, id_destino]  # Mantém a taxa para conversão de moeda estrangeira para Real
        valor_final = valor * taxa # Se a origem for ESTRANGEIRA, nós MULTIPLICAMOS para virar Real
...
while True:
    ...
    if comecar.upper() == "N":
        ...
        try:
            ...
            print("Moedas disponíveis: 1-Dólar / 2-Euro / 3-Libra / 4-Yuan / 5-Won / 6-Real")
            ...
            print("Converter para: 1-Dólar / 2-Euro / 3-Libra / 4-Yuan / 5-Won / 6-Real")
